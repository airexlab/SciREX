...
from scirex.nn.layers import Dropout, Lambda, Linear, Sequential
...
def load_mnist_data(batch_size: int = 128) -> tuple[tfds.data.Dataset, tfds.data.Dataset]:
    ...
    print(
        f"✓ Dataset loaded: {ds_builder.info.splits['train'].num_examples} train, "
...
def create_model(rngs: nnx.Rngs) -> Sequential:
    ...
    print("=" * 60)
    print("Input:  784 features (28x28 flattened)")
...
def visualize_results(
    history: dict[str, list[float]], model: Sequential, test_ds, save_path: str = "mnist_results.png"
):
    """
    ...
    for x, y in test_ds.take(1):
        ...
        for i in range(6):
            ...
            ax.set_title(f"True: {true_label}, Pred: {pred_label}", color=color, fontsize=12, fontweight="bold")
            ax.axis("off")
