...
import pytest
from scirex.experimental.sciml.fe.fe2d_setup_main import FE2DSetupMain
...
@pytest.mark.parametrize("fe_type", ["legendre", "jacobi", "legendre_special", "chebyshev_2", "jacobi_plain"])
...
def test_valid_transformation_type(transformation_type):
    ...
    assert fe2d_setup_main.assign_fe_transformation(transformation_type, [[0, 0], [1, 0], [1, 1], [0, 1]]) is not None
...
def test_invalid_transformation_type():
    ...
    with pytest.raises(ValueError):
        ...
        fe2d_setup_main.assign_fe_transformation("invalid_transformation", [[0, 0], [1, 0], [1, 1], [0, 1]])
